...
class GstUvMxnetSharp(GstBase.BaseTransform):

    GST_PLUGIN_NAME = 'uv_sharp_py'
    ...
    def __init__(self):
        ...
        super(GstUvMxnetSharp, self).__init__()
        self.minimal = 0
        self.kernel_size = 2
        self.contrast = False
...
GObject.type_register(GstUvMxnetSharp)
__gstelementfactory__ = (GstUvMxnetSharp.GST_PLUGIN_NAME,
                         Gst.Rank.NONE, GstUvMxnetSharp)
...
def histContrast(img: np.ndarray) ->  np.ndarray :
  ...
  cdf = np.cumsum(h)
  indexes = np.nonzero(cdf)
  minCdf = cdf[indexes[0][0]]
  div = (img.size-1.0)
